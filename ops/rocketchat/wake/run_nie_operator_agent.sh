#!/usr/bin/env bash
# KeepAlive launchd entry: NIE (z-mapping novel insight) Hermes presence.
set -euo pipefail
...
export RC_SECRETS_PATH="${RC_SECRETS_PATH:-$HOME/.grok/agency/secrets/rocketchat-nie.env}"
export RC_LOG_DIR="${RC_LOG_DIR:-$HOME/logs/rocketchat-nie-wake}"
export RC_STATE_PATH="${RC_STATE_PATH:-$OPS_RC/wake/nie_state.json}"
export RC_REPLY_PROMPT="${RC_REPLY_PROMPT:-$OPS_RC/wake/nie_reply_prompt.txt}"

# --- backend: Hermes CLI on profile nie ---
export RC_WAKE_BACKEND=hermes
export RC_HERMES_PROFILE="${RC_HERMES_PROFILE:-nie}"
export RC_WAKE_STREAM="${RC_WAKE_STREAM:-0}"

# Dual-operator: shared channels/groups only wake on @nie; DMs free-wake.
